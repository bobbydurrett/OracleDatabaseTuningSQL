...
spool inoneblock.log

-- find one block blob cutoff point in bytes
-- for basicfile blob
...
if full_blocks <= 1 then
    dbms_output.put_line('in one block');
else
    dbms_output.put_line('not in one block');
end if;
