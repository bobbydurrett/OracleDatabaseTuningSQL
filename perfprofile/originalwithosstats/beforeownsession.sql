...
drop table beforeownsession;
create table beforeownsession as
select SID,event TIMESOURCE,(TIME_WAITED_MICRO/1000000) seconds from v$session_event where sid=(SELECT sid from v$session where audsid=USERENV('SESSIONID'));
insert into beforeownsession select SID,'CPU' TIMESOURCE,(VALUE/100) seconds from v$sesstat where sid=(SELECT sid from v$session where audsid=USERENV('SESSIONID'))
and statistic#=(select statistic# from v$statname where name='CPU used by this session');
commit;
insert into beforeownsession
select sid,'OSWASTED' TIMESOURCE,sum(value)/100 seconds
from v$sesstat where sid=(SELECT sid from v$session where audsid=USERENV('SESSIONID'))
and statistic# in
